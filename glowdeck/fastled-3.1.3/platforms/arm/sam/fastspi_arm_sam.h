...
class SAMHardwareSPIOutput
{
    ...
    static inline void waitForEmpty() {
        while ((m_SPI->SPI_SR & SPI_SR_TDRE) == 0);
    }

    void enableConfig() {
        m_SPI->SPI_WPMR &= ~SPI_WPMR_WPEN;
    }
    void disableConfig() {
        m_SPI->SPI_WPMR |= SPI_WPMR_WPEN;
    }
    ...
    void enableSPI() {
        m_SPI->SPI_CR = SPI_CR_SPIEN;
    }
    void disableSPI() {
        m_SPI->SPI_CR = SPI_CR_SPIDIS;
    }
    void resetSPI() {
        m_SPI->SPI_CR = SPI_CR_SWRST;
    }
  ...
  public:
    ...
    void waitFully() {
        while ((m_SPI->SPI_SR & SPI_SR_TXEMPTY) == 0);
    }
