...
template<uint8_t PIN, uint8_t _MASK, typename _PORT, typename _DDR, typename _PIN> class _AVRPIN
{
  public:
    ...
    inline static void set(register uint8_t val) __attribute__((always_inline)) {
        _PORT::r() = val;
    }
    ...
    inline static void fastset(register port_ptr_t /*port*/, register uint8_t val) __attribute__((always_inline)) {
        set(val);
    }
